...
use App\Controller\AjouterTypeDirectionAction;
use App\Controller\Delete\DeleteTypeDirectionAction;
...
#[ApiResource(
    normalizationContext: ['groups' => ['read:TypeDirection','read:Entity']],
    denormalizationContext: ['groups' => ['write:TypeDirection','write:Entity']],
    operations: [
        new Get(),
        new GetCollection(),
        new Post(
            controller: AjouterTypeDirectionAction::class,
            write: false,
...
//        ),
        new Delete(
            security: "is_granted('ROLE_ADMIN')",
            controller: DeleteTypeDirectionAction::class,
            write: false
        ),
    ]
)]
...
class TypeDirection implements UserOwnedInterface
{
    ...
    #[ORM\OneToMany(mappedBy: 'typeDirection', targetEntity: Direction::class)]
    ...
    public function addDirection(Direction $direction): static
    {
        if (!$this->directions->contains($direction)) {
            ...
            $direction->setTypeDirection($this);
        }
    ...
    public function removeDirection(Direction $direction): static
    {
        if ($this->directions->removeElement($direction)) {
            ...
            if ($direction->getTypeDirection() === $this) {
                $direction->setTypeDirection(null);
            }
        }
